            ...
            data: (penjualanList) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final motorAsync = ref.watch(motorByIdProvider(p.motorId));
                  return Card(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // V5.5 — plat nomor + detail motor (bukan kode).
                          motorAsync.when(
                            data: (m) => m == null
                                ...
                                : Padding(
                                    ...
                                    child: Text(
                                      '${m.platNomor?.isNotEmpty == true ? m.platNomor : "(Plat -)"} • ${m.namaLengkap}',
                                      style: const TextStyle(
                          ...
                          if (p.biayaCalo > 0) ...[
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                const Text('Biaya Calo',
                                    style: TextStyle(color: Colors.orange)),
                                Text('- ${AppFormatter.rupiah(p.biayaCalo)}',
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                const Text('Hasil Bersih'),
                                Text(AppFormatter.rupiah(p.hasilBersih)),
                              ],
                            ),
                          ],
                          const SizedBox(height: 4),
...
class _JualMotorSheetState extends ConsumerState<_JualMotorSheet> {
  ...
  final _biayaCaloController = TextEditingController();
  ...
  double get _biayaCalo =>
      double.tryParse(_biayaCaloController.text.replaceAll('.', '')) ?? 0;
  double get _hasilBersih => (_hargaJual - _biayaCalo).clamp(0, double.infinity);
  ...
  Future<void> _simpan() async {
    ...
    if (_biayaCalo < 0 || _biayaCalo > _hargaJual) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Biaya calo harus antara 0 dan harga jual')));
      return;
    }
            ...
            child: ListView(
              ...
              children: [
                    ...
                    return DropdownButtonFormField<int>(
                      ...
                      items: stok
                          .map((m) => DropdownMenuItem(
                                value: m.id,
                                // V5.5 — plat nomor + detail motor, TANPA kode
                                // motor (banyak unit tipe/tahun sama, plat
                                // yang membedakan & mudah dicari penjual).
                                child: Text(
                                    '${m.platNomor?.isNotEmpty == true ? m.platNomor : "(Plat -)"} • ${m.namaLengkap}'),
                              ))
                          .toList(),
                ...
                if (isCalo)
                  Container(
                    ...
                    child: const Text(
                      ...
                      'Isi "Biaya Calo" di bawah kalau ada fee yang harus '
                      'dipotong dari hasil penjualan.',
                      style: TextStyle(fontSize: 12, color: Colors.orange),
                    ),
                  ),
                const SizedBox(height: 12),
                TextFormField(
                  controller: _biayaCaloController,
                  decoration: const InputDecoration(
                    labelText: 'Biaya Calo (opsional)',
                    prefixText: 'Rp ',
                    helperText:
                        'Dipotong dari harga jual, laba otomatis berkurang.',
                  ),
                  keyboardType: TextInputType.number,
                  inputFormatters: [RupiahInputFormatter()],
                  onChanged: (_) => setState(() {}),
                ...
                if (_biayaCalo > 0) ...[
                  const SizedBox(height: 4),
                  Text(
                    'Hasil penjualan bersih: ${AppFormatter.rupiah(_hasilBersih)} '
                    '(${AppFormatter.rupiah(_hargaJual)} - ${AppFormatter.rupiah(_biayaCalo)})',
