        ...
        data: (periode) {
          ...
          return daftarAsync.when(
            ...
            data: (list) {
              ...
              // V5.7 — FIX: dulu halaman ini cuma menampilkan total manual
              // dan mengklaim "biaya dari Pengeluaran terhitung otomatis"
              // tanpa benar-benar menampilkan angkanya, PADAHAL biaya
              // transfer juga tersimpan di Motor, Biaya Motor, dan Dana
              // Talang. Sekarang tampilkan juga Total Gabungan (semua
              // sumber, sudah benar dihitung LaporanService) supaya
              // kelihatan utuh, bukan cuma manual.
              final totalGabunganAsync = periode?.id != null
                  ? ref.watch(laporanPeriodeProvider(periode!.id!))
                  : null;
              return Column(
                children: [
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (totalGabunganAsync != null) ...[
                          const Divider(height: 20),
                          totalGabunganAsync.when(
                            loading: () => const SizedBox(
                                height: 16,
                                width: 16,
                                child: CircularProgressIndicator(strokeWidth: 2)),
                            error: (e, st) => Text('Gagal memuat total gabungan: $e',
                            ...
                            data: (lap) => Row(
                              ...
                              children: [
                                const Expanded(
                                  child: Text(
                                    'Total Biaya Transfer Gabungan\n(Pengeluaran, Motor, Biaya Motor, Dana Talang, Kasbon, Manual)',
                                    style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12),
                                  ),
                                ),
                                Text(
                                  AppFormatter.rupiah(lap.totalBiayaTransferGabungan),
                                  style: const TextStyle(
                                      fontWeight: FontWeight.w800,
                                      color: AppTheme.danger,
                                      fontSize: 16),
                  ...
                  // V5.8 — rincian per-transaksi biaya transfer dari semua
                  // sumber (Pengeluaran, Motor, Biaya Motor, Dana Talang,
                  // Kasbon) yang membentuk "Total Gabungan" di atas —
                  // sebelumnya totalnya sudah benar tapi tidak ada rincian
                  // per transaksi sama sekali.
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    child: Consumer(builder: (context, ref, _) {
                      final detailAsync = ref.watch(
                          biayaTransferDetailProvider(periode?.id));
                      return detailAsync.when(
                        loading: () => const Padding(
                          padding: EdgeInsets.symmetric(vertical: 8),
                          child: LinearProgressIndicator(),
                        ),
                        error: (e, st) => Text('Gagal memuat rincian: $e',
                            style: const TextStyle(fontSize: 11)),
                        data: (items) => Theme(
                          ...
                          child: ExpansionTile(
                            ...
                            title: Text(
                              'Rincian dari Transaksi Lain (${items.length})',
                              style: const TextStyle(
                            ...
                            children: [
                              if (items.isEmpty)
                                const Padding(
                                  ...
                                  child: Text(
                                    'Belum ada biaya transfer dari transaksi lain.',
                                        ...
                                        children: [
                                          Expanded(
                                            child: Column(
                                              ...
                                              children: [
                                                Text(
                                                  '${AppFormatter.tanggal(it.tanggal)} • ${it.sumber}',
                                                  style: const TextStyle(
                                          ...
                                          Text(
                                            '-${AppFormatter.rupiah(it.nominal)}',
                                            style: const TextStyle(
                                                fontSize: 12,
                  ...
                  const Padding(
                    ...
                    child: Align(
                      ...
                      child: Text(
                        'Daftar di bawah HANYA untuk biaya transfer manual '
                        '(tidak terkait transaksi manapun). Rincian dari '
                        'transaksi lain ada di panel "Rincian dari Transaksi Lain" di atas.',
