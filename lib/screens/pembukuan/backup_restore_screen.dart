...
class _BackupRestoreScreenState extends State<BackupRestoreScreen> {
  ...
  Widget build(BuildContext context) {
    if (!AuthService.instance.isOwnerAdmin) {
      return Scaffold(
  ...
  Future<void> _backup() async {
    ...
    try {
      ...
      final fileName = 'garasi_abah_bontot_backup_$timestamp.db';
      ...
      // V5.6 — sebelumnya HANYA bisa "Bagikan" (share sheet), tidak ada
      // opsi simpan langsung ke penyimpanan perangkat. Sekarang tanya dulu
      // mau simpan langsung atau bagikan (dua-duanya tetap tersedia).
      if (!mounted) return;
      final pilihan = await showDialog<String>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Backup Berhasil Dibuat'),
          content: const Text('Mau simpan ke mana?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, 'simpan'),
              child: const Text('Simpan ke Perangkat'),
            ),
            TextButton(
              onPressed: () => Navigator.pop(context, 'bagikan'),
      ...
      if (pilihan == 'simpan') {
        ...
        setState(() => _statusMessage = savedPath != null
            ? 'Backup tersimpan di perangkat.'
            : 'Penyimpanan dibatalkan.');
      } else if (pilihan == 'bagikan') {
        await Share.shareXFiles(
          [XFile(file.path)],
          text: 'Backup database Garasi Abah Bontot - $timestamp',
        );
        setState(() => _statusMessage = 'Backup berhasil dibuat & dibagikan.');
      } else {
        setState(() => _statusMessage =
            'Backup dibuat di cache aplikasi (belum disimpan/dibagikan).');
      }
    } catch (e) {
