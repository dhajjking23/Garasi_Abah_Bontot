...
import '../gajihan/gajihan_screen.dart';
...
import '../closing/closing_period_screen.dart';
      ...
      body: ListView(
        ...
        children: StaggeredEntrance.staggeredChildren([
          ...
          _MenuTile(
            ...
            label: 'Dana Talang Partner',
            subtitle: 'Piutang & hutang antar partner',
            onTap: () => Navigator.push(context,
                MaterialPageRoute(builder: (_) => const DanaTalangScreen()))
                .then((_) => refreshSemuaData(ref)),
          ),
          _MenuTile(
            icon: Icons.payments_outlined,
            label: 'Gajihan & Tutup Buku',
            subtitle: 'Proses gaji dengan potongan kasbon/dana talang',
            onTap: () => Navigator.push(context,
                MaterialPageRoute(builder: (_) => const GajihanScreen()))
                .then((_) => refreshSemuaData(ref)),
          ),
          _MenuTile(
            icon: Icons.fact_check_outlined,
            label: 'Closing Period',
            subtitle: 'Rekonsiliasi & snapshot neraca (bukan kunci transaksi)',
            onTap: () {
              final periode = periodeAktifAsync.value;
              if (periode?.id == null) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text('Belum ada periode aktif.')));
                return;
              }
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => ClosingPeriodScreen(periodeId: periode!.id!)),
              );
            },
          ),
          _MenuTile(
