          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Diatur sekali, tersimpan permanen. Mode LOCAL: Partner auto-'
                  'discovery di WiFi yang sama, tidak perlu input manual. Mode '
                  'VPS: Partner WAJIB isi Host/Port/Token yang SAMA PERSIS di '
                  'HP masing-masing lewat menu Server mereka sendiri.',
...
/// V5.4 — Partner sekarang bisa input konfigurasi server sendiri. Auto-
/// discovery (scan subnet WiFi) HANYA berlaku untuk mode LOCAL yang satu
/// jaringan dengan server; untuk mode VPS (internet), tidak ada cara
/// "menemukan" server tanpa host eksplisit — jadi Partner WAJIB isi
/// Host/Port/Token manual, sekali, tersimpan permanen di HP-nya sendiri
/// (SharedPreferences per-device — TIDAK otomatis ikut config Admin,
/// karena keduanya device terpisah tanpa jalur berbagi config).
class _ViewerView extends ConsumerStatefulWidget {
  final _ServerScreenState screen;
  const _ViewerView({required this.screen});

  @override
  ConsumerState<_ViewerView> createState() => _ViewerViewState();
}

class _ViewerViewState extends ConsumerState<_ViewerView> {
  late TextEditingController _hostCtrl;
  late TextEditingController _portCtrl;
  late TextEditingController _tokenCtrl;
  late ServerMode _mode;
  ...
  void _initFromConfig(ServerConfig config) {
  ...
  Future<void> _save() async {
    ...
    setState(() => _saving = false);
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Konfigurasi server disimpan di HP ini.')),
    );
    await widget.screen._checkStatus(newConfig);
  }
      ...
      children: [
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Konfigurasi Koneksi Server',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                const Text(
                  'Untuk mode LOCAL (satu WiFi dengan server), boleh dikosongkan — '
                  'sistem cari otomatis. Untuk mode VPS (server di internet), '
                  'WAJIB isi Host/Domain dari Admin, sekali saja — tersimpan '
                  'permanen di HP ini.',
                  style: TextStyle(fontSize: 12, color: Colors.grey),
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<ServerMode>(
                  ...
                  items: const [
                    DropdownMenuItem(value: ServerMode.local, child: Text('LOCAL (WiFi/LAN sama)')),
                    DropdownMenuItem(value: ServerMode.vps, child: Text('VPS (server jarak jauh)')),
                  ],
                  onChanged: (v) => setState(() => _mode = v ?? ServerMode.local),
                ),
                ...
                TextField(
                  controller: _hostCtrl,
                  decoration: InputDecoration(
                    labelText: _mode == ServerMode.local
                        ? 'IP Terakhir (opsional, auto-discovery aktif)'
                        : 'Host / Domain VPS (minta ke Admin)',
                    helperText: _mode == ServerMode.local
                        ? 'Boleh dikosongkan — sistem akan cari otomatis di jaringan WiFi lokal.'
                        : 'Contoh: garasi.namadomain.com atau 203.0.113.10 -- HARUS sama dengan yang dipakai Admin.',
                    border: const OutlineInputBorder(),
                  ),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'API Token',
                    helperText: 'HARUS sama persis dengan token yang dipakai Admin.',
                    border: OutlineInputBorder(),
                  ),
                  obscureText: true,
                ),
        ...
        const Card(
          child: Padding(
            padding: EdgeInsets.all(16),
            child: Text(
              'Koneksi ke server dicari & disambungkan otomatis setelah '
              'konfigurasi disimpan. Jika OFFLINE, aplikasi tetap bisa '
              'dipakai memakai data terakhir yang tersimpan di HP ini.',
            ),
          ),
        ),
      ],
