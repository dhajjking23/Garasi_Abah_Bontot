...
class PenjualanRepository {
  ...
  Future<PenjualanModel> jualMotor({
    ...
    /// V5.5 — biaya calo/perantara, dipotong dari harga jual sebelum laba
    /// dihitung. Default 0 (perilaku lama tidak berubah untuk penjualan
    /// tanpa calo). Tidak mengubah hargaJual/piutang yang ditagih ke
    /// pembeli — murni pengurang laba.
    double biayaCalo = 0,
    ...
    return db.transaction<PenjualanModel>((txn) async {
      ...
      if (biayaCalo < 0 || biayaCalo > hargaJual) {
        throw ArgumentError('Biaya calo harus antara 0 dan harga jual');
      }
      final laba = hargaJual - biayaCalo - modalMotor;
      ...
      await auditLog.catatCreate(
        ...
        keterangan: 'Jual ${motor.kodeMotor} oleh $penjual ($metodePembayaran)'
            ...
            '${biayaCalo > 0 ? " - Biaya Calo Rp${biayaCalo.toStringAsFixed(0)}" : ""}',
      );
  ...
  Future<PenjualanModel> editPenjualan({
    ...
    return db.transaction<PenjualanModel>((txn) async {
      ...
      final labaBaru = hargaJualBaru - lama.biayaCalo - lama.modalMotor;
