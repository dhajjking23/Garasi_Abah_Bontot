...
class SaldoRepository {
  ...
  /// V5.9.9 — Penyesuaian Modal Awal periode baru (carry-forward), kalau
  /// user EDIT total modal awal dari nilai otomatis (Total Aset live).
  ///
  /// [selisih] = modalAwalFinal - totalAsetOtomatis. POSITIF artinya
  /// modal awal DINAIKKAN -> suntik Cash/Bank SUNGGUHAN masuk + Modal
  /// naik (uang baru sungguhan, mis. owner nambah modal). NEGATIF
  /// artinya modal awal DITURUNKAN -> tarik Cash/Bank SUNGGUHAN keluar +
  /// Modal turun (prive/penarikan modal).
  ///
  /// [cash]/[bank] adalah nilai MUTLAK (bukan signed) sesuai metode
  /// pembayaran yang dipilih user; harus `cash + bank == selisih.abs()`
  /// (divalidasi). TIDAK ADA fake balancing -- kalau mau menaikkan modal
  /// awal, uang itu harus benar-benar masuk ke Cash/Bank, bukan cuma
  /// angka modal_awal_total_aset yang diubah sendirian.
  Future<SaldoModel> sesuaikanModalAwalPeriode({
    required double selisih,
    required double cash,
    required double bank,
    String? keterangan,
  }) async {
    if (selisih.abs() < 0.5) return getSaldo();
    final totalPorsi = cash + bank;
    if ((totalPorsi - selisih.abs()).abs() > 0.5) {
      throw ArgumentError(
          'Cash + Bank (Rp${totalPorsi.toStringAsFixed(0)}) harus sama dengan '
          'selisih modal awal (Rp${selisih.abs().toStringAsFixed(0)})');
    }

    final naik = selisih > 0;
    final label = naik ? 'Penyesuaian Modal Awal (+)' : 'Penyesuaian Modal Awal (-)';

    if (!naik) {
      final saldo = await getSaldo();
      if (cash > saldo.cash + 0.5) {
        throw StateError(
            'Cash tidak cukup untuk menurunkan modal awal sejumlah ini '
            '(Cash tersedia Rp${saldo.cash.toStringAsFixed(0)}, dibutuhkan Rp${cash.toStringAsFixed(0)}).');
      }
      if (bank > saldo.saldoBank + 0.5) {
        throw StateError(
            'Saldo Bank tidak cukup untuk menurunkan modal awal sejumlah ini '
            '(Bank tersedia Rp${saldo.saldoBank.toStringAsFixed(0)}, dibutuhkan Rp${bank.toStringAsFixed(0)}).');
      }
    }
    ...
    if (cash > 0) {
      await mutasiCash(
        nominal: cash,
        tipe: naik ? AppConstants.cashFlowMasuk : AppConstants.cashFlowKeluar,
        referensi: AppConstants.cashFlowRefModalAwalPeriode,
        keterangan: keterangan ?? '$label - Cash',
      );
      await ubahModal(
        jenis: AppConstants.modalJenisCash,
        aksi: naik ? AppConstants.modalAksiTambah : AppConstants.modalAksiKurang,
        nominal: cash,
        keterangan: keterangan ?? '$label - Cash',
      );
    }
    if (bank > 0) {
      await mutasiBank(
        ...
        tipe: naik ? AppConstants.cashFlowMasuk : AppConstants.cashFlowKeluar,
        referensi: AppConstants.cashFlowRefModalAwalPeriode,
        keterangan: keterangan ?? '$label - Bank',
      );
      await ubahModal(
        jenis: AppConstants.modalJenisBank,
        aksi: naik ? AppConstants.modalAksiTambah : AppConstants.modalAksiKurang,
