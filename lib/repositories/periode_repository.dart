    ...
    return db.transaction<PeriodeModel>((txn) async {
      ...
      await auditLog.catatCreate(
        ...
        keterangan: 'Buka periode baru (carry-forward). Modal Awal (Total '
            'Aset) = Rp${totalAsetAkhirClosing.toStringAsFixed(0)}'
            '${selisihModal != 0 ? " (sudah termasuk penyesuaian manual Rp${selisihModal.toStringAsFixed(0)})" : ", sama persis dengan total aset akhir closing sebelumnya (selisih Rp0)"}, '
            'sudah divalidasi PeriodOpeningService sebelum method ini '
            'dipanggil. Cash, Bank, Stok, dan Piutang carry-forward '
            'otomatis dari sebelum periode ini dibuka'
            '${selisihModal != 0 ? "; HANYA selisih penyesuaian manual di atas yang benar-benar menyuntik/menarik Cash/Bank." : "."}',
      );

      // V5.9.9 — kalau user EDIT Modal Awal dari nilai otomatis, selisih
      // itu HARUS benar-benar disuntik/ditarik dari Cash/Bank sungguhan
      // (bukan cuma angka modal_awal_total_aset yang diubah sendirian —
      // itu akan jadi fake balancing). Dilakukan DI DALAM transaction
      // yang sama supaya periode baru + penyesuaian saldo atomic.
      if (selisihModal.abs() >= 0.5) {
        final saldoRepo = SaldoRepository(txn);
        await saldoRepo.sesuaikanModalAwalPeriode(
          selisih: selisihModal,
          cash: cashSelisih,
          bank: bankSelisih,
          keterangan: 'Penyesuaian Modal Awal periode "$namaPeriode" '
              '(${selisihModal > 0 ? "tambahan modal" : "penarikan modal"}, '
              'metode $metodePembayaranSelisih)',
        );
        await auditLog.catatCreate(
          'periode',
          id,
          jsonEncode({'selisih_modal': selisihModal, 'cash': cashSelisih, 'bank': bankSelisih}),
          keterangan: 'Penyesuaian Modal Awal periode baru: '
              '${selisihModal > 0 ? "+" : ""}Rp${selisihModal.toStringAsFixed(0)} '
              'via $metodePembayaranSelisih -- Cash/Bank BENAR-BENAR '
              '${selisihModal > 0 ? "bertambah" : "berkurang"} sejumlah ini.',
        );
      }
