...
class PengeluaranRepository {
  ...
  Future<PengeluaranModel> tambahPengeluaran({
    ...
    requireWriteAccess();
    return db.transaction<PengeluaranModel>((txn) => tambahPengeluaranInTxn(
          txn,
          tanggal: tanggal,
          kategori: kategori,
          nominal: nominal,
  ...
  /// V5.9.2 — Sama persis dengan [tambahPengeluaran], TAPI menerima
  /// [txn] dari LUAR (bukan membuka `db.transaction()` sendiri). Dipakai
  /// oleh operasi majemuk yang butuh SATU transaction gabungan lintas
  /// repository (mis. `GajihanRepository.prosesGajihan()` — supaya gaji
  /// + pelunasan kasbon + pelunasan dana talang sungguh-sungguh
  /// ALL-OR-NOTHING, bukan 3 transaction terpisah). `requireWriteAccess()`
  /// TETAP dicek di sini (tidak boleh dilewati hanya karena dipanggil
  /// dari repo lain).
  Future<PengeluaranModel> tambahPengeluaranInTxn(
