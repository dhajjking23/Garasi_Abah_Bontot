...
class KasbonRepository {
  ...
  Future<KasbonModel> bayarSebagianKasbon({
    ...
    requireWriteAccess();
    return db.transaction<KasbonModel>((txn) =>
        bayarSebagianKasbonInTxn(txn, kasbonId: kasbonId, nominal: nominal, tanggal: tanggal));
  }

  /// V5.9.9 — sama persis dengan [bayarSebagianKasbon], TAPI menerima
  /// [txn] dari luar. Dipakai `GajihanRepository.prosesGajihan()` supaya
  /// kasbon yang LEBIH BESAR dari potongan gajihan periode ini tetap
  /// bisa DICICIL (bukan terlewat sepenuhnya) sambil tetap dalam SATU
  /// transaction atomic dengan gaji+dana talang.
  Future<KasbonModel> bayarSebagianKasbonInTxn(
