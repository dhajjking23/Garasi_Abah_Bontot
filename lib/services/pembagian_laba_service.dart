...
class PembagianLabaService {
  ...
  Future<PembagianLabaModel> tutupBukuDanBagiLaba(
    ...
    if (totalAkanDibayar > 0) {
      ...
      if (poolCash > saldo.cash + 0.5) {
        throw StateError(
            'INSUFFICIENT_CASH: Porsi Cash yang diminta Rp${poolCash.toStringAsFixed(0)} '
            'melebihi Cash tersedia Rp${saldo.cash.toStringAsFixed(0)}. '
            'Tutup buku DIBATALKAN, tidak ada perubahan yang tersimpan.');
      }
      if (poolBank > saldo.saldoBank + 0.5) {
        throw StateError(
            'INSUFFICIENT_CASH: Porsi Bank yang diminta Rp${poolBank.toStringAsFixed(0)} '
            'melebihi Saldo Bank tersedia Rp${saldo.saldoBank.toStringAsFixed(0)}. '
            'Tutup buku DIBATALKAN, tidak ada perubahan yang tersimpan.');
      }
    }
