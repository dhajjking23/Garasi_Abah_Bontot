...
class LaporanPeriodeData {
  ...
  /// V5.9.2 — PERINGATAN: ini BUKAN laba (lihat Mega Prompt §4/§15,
  /// Invariant 2/3 — "TOTAL ASSET - MODAL = LABA" eksplisit dilarang).
  /// Field ini dipertahankan HANYA sebagai indikator diagnostik selisih
  /// (untuk membantu mendeteksi transaksi yang mungkin belum tercatat),
  /// BUKAN sebagai komponen laba periode. `FinancialBalanceEngine` TIDAK
  /// LAGI memakai nilai ini untuk laba periode berjalan — lihat
  /// `FinancialBalanceEngine.calculatePeriodProfit()` yang memakai
  /// `PembagianLabaService.hitungPreview()` (Revenue - COGS - Biaya)
  /// sebagai gantinya. UI juga sudah diberi label "diagnostik, BUKAN
  /// laba", bukan "Laba (Perubahan Modal)" seperti sebelumnya.
  ///
  /// Juga PERLU DIKETAHUI (pending, belum diperbaiki): `totalAset` di
  /// bawah ini dihitung dari SALDO GLOBAL SAAT INI (bukan "as of" akhir
  /// periode), sedangkan `modalAwalPeriode` adalah nilai PERIOD-SPECIFIC
  /// — mencampur GLOBAL vs PERIOD (Mega Prompt Section D). Memperbaikinya
  /// dengan benar butuh ledger point-in-time (jumlah cash_flow sampai
  /// tanggal tertentu), yang belum ada di scope perbaikan ini.
