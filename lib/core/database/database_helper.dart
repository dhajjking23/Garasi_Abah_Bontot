...
class DatabaseHelper {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    batch.execute('''
      CREATE TABLE periode (
        ...
        modal_awal_total_aset REAL NOT NULL DEFAULT 0,
    ...
    batch.execute('''
      CREATE TABLE penjualan (
        ...
        biaya_calo REAL NOT NULL DEFAULT 0,
    ...
    // ==========================================================
    // TABEL: closing_snapshot (V5.9)
    // "Foto" hasil rekonsiliasi pada saat tutup buku/rekalkulasi.
    // BUKAN period lock -- transaksi tetap bisa diedit kapan saja.
    // ==========================================================
    batch.execute('''
      CREATE TABLE closing_snapshot (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        periode_id INTEGER NOT NULL,
        ...
        total_modal_motor REAL NOT NULL DEFAULT 0,
        ...
        kasbon REAL NOT NULL DEFAULT 0,
        hutang REAL NOT NULL DEFAULT 0,
        dana_talang REAL NOT NULL DEFAULT 0,
        total_asset REAL NOT NULL DEFAULT 0,
        total_liability REAL NOT NULL DEFAULT 0,
        total_equity REAL NOT NULL DEFAULT 0,
        balance_difference REAL NOT NULL DEFAULT 0,
        closing_status TEXT NOT NULL DEFAULT 'BALANCE',
        created_at TEXT NOT NULL,
        created_by TEXT
      )
    ''');
    batch.execute(
        'CREATE INDEX idx_closing_snapshot_periode ON closing_snapshot(periode_id)');
    ...
    batch.execute('''
      CREATE TABLE dana_talang (
        ...
        total_dikonversi_modal REAL NOT NULL DEFAULT 0,
        is_archived INTEGER NOT NULL DEFAULT 0,
        keterangan TEXT,
        periode_id INTEGER,
        created_at TEXT NOT NULL,
    ...
    // ==========================================================
    // TABEL: dana_talang_pembayaran (riwayat cicilan/pelunasan/konversi)
    // ==========================================================
    batch.execute('''
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 17) {
      // V17 — GARASI ABAH BONTOT V5.5: fitur "Biaya Calo" di penjualan.
      // Kolom baru, default 0 -- semua penjualan lama otomatis
      // biaya_calo=0 (laba tercatat mereka TIDAK berubah sama sekali).
      await _addColumnIfNotExists(
          db, 'penjualan', 'biaya_calo', 'REAL NOT NULL DEFAULT 0');
    }

    if (oldVersion < 18) {
      // V18 — GARASI ABAH BONTOT V5.9: Closing Snapshot (BUKAN period
      // lock — tabel ini HANYA "foto" hasil perhitungan pada saat
      // tutup buku/rekalkulasi dijalankan, transaksi TETAP bisa diedit
      // kapan saja setelahnya). Lihat services/closing_snapshot_service.dart.
      await db.execute('''
      ...
      ''');
      await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_closing_snapshot_periode ON closing_snapshot(periode_id)');
    }

    if (oldVersion < 19) {
      // V19 — GARASI ABAH BONTOT V5.9.1: Dana Talang -> CAPITAL
      // CONVERSION (Mega Prompt §11/§12, P0-2). Kolom baru, TERPISAH dari
      // total_dibayar_kembali supaya "dibayar cash" vs "dikonversi jadi
      // modal" tetap bisa dibedakan di laporan (Spec Koreksi §7: NETTING
      // / PAYMENT / CAPITAL CONVERSION adalah 3 hal berbeda). Default 0
      // untuk semua baris lama -- TIDAK mengubah sisa/status dana talang
      // yang sudah ada (perilaku lama: sisa = nominal - total_dibayar_kembali
      // tetap sama persis kalau total_dikonversi_modal = 0).
      // Lihat DanaTalangRepository.konversiKeModal().
      await _addColumnIfNotExists(
          db, 'dana_talang', 'total_dikonversi_modal', 'REAL NOT NULL DEFAULT 0');
    }

    if (oldVersion < 20) {
      // V20 — GARASI ABAH BONTOT V5.9.2: Kasbon PARTIAL REPAYMENT
      // (Mega Prompt Section H). Kolom baru, additive, default 0 —
      // baris lama tetap berperilaku sama persis (outstanding =
      // jumlah - 0 = jumlah, status tidak berubah) sampai ada pembayaran
      // sebagian baru lewat KasbonRepository.bayarSebagianKasbon().
      await _addColumnIfNotExists(
          db, 'kasbon', 'total_dibayar', 'REAL NOT NULL DEFAULT 0');
    }

    if (oldVersion < 21) {
      // V21 — GARASI ABAH BONTOT V5.9.4: (1) Closing membayar gajihan &
      // bonus penjualan secara nyata (Section C/I dari permintaan user),
      // (2) Opening period carry-forward yang benar (modal awal = total
      // aset akhir closing, BUKAN cash injection baru), (3) Arsip
      // (bukan hapus) transaksi kasbon/dana talang yang sudah lunas saat
      // buka periode baru, supaya daftar tidak menumpuk tapi histori
      // tetap ada di database untuk audit.
      await _addColumnIfNotExists(
          db, 'periode', 'modal_awal_total_aset', 'REAL NOT NULL DEFAULT 0');
      await _addColumnIfNotExists(
          db, 'kasbon', 'is_archived', 'INTEGER NOT NULL DEFAULT 0');
      await _addColumnIfNotExists(
          db, 'dana_talang', 'is_archived', 'INTEGER NOT NULL DEFAULT 0');
    }

    if (oldVersion < 22) {
      // V22 — GARASI ABAH BONTOT V5.9.5: gaji standar per karyawan
      // (Gajihan Screen), supaya "Gaji Pokok" tidak perlu diketik ulang
      // manual tiap proses gajihan -- auto-fill dari nilai tersimpan,
      // tetap bisa diedit per-transaksi. Additive, default 0 (perilaku
      // lama: field kosong) untuk baris karyawan yang sudah ada.
      await _addColumnIfNotExists(
          db, 'karyawan', 'gaji_pokok', 'REAL NOT NULL DEFAULT 0');
    }
  }
