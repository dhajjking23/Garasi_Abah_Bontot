...
class DanaTalangModel extends Equatable {
  ...
  final String status; // BELUM_LUNAS / SEBAGIAN_LUNAS / LUNAS / BATAL / SEBAGIAN_DIKONVERSI / DIKONVERSI_MODAL
  final double totalDibayarKembali;
  /// V5.9.1 — total yang sudah dikonversi jadi Tambahan Modal (BUKAN
  /// pembayaran cash). Dipisah dari [totalDibayarKembali] supaya
  /// NETTING/PAYMENT/CAPITAL CONVERSION tetap bisa dibedakan di laporan.
  /// Cash/Bank TIDAK PERNAH berubah karena nilai ini.
  final double totalDikonversiModal;
  /// V5.9.4 — arsip (BUKAN hapus), lihat KasbonModel.isArchived.
  ...
  bool get isAktif =>
      ...
      status != AppConstants.statusDanaTalangDikonversiModal;
  /// Sisa outstanding = nominal dikurangi SEMUA pelunasan, baik lewat
  /// cash (totalDibayarKembali) MAUPUN lewat konversi modal
  /// (totalDikonversiModal) — keduanya sama-sama mengurangi hutang,
  /// bedanya cuma di sisi mana yang berubah (Cash vs Modal).
  double get sisa => status == AppConstants.statusDanaTalangBatal
      ? 0
      : (nominal - totalDibayarKembali - totalDikonversiModal)
          .clamp(0, nominal);
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'total_dikonversi_modal': totalDikonversiModal,
  ...
  factory DanaTalangModel.fromMap(Map<String, dynamic> map) {
    return DanaTalangModel(
      ...
      totalDikonversiModal:
          (map['total_dikonversi_modal'] as num?)?.toDouble() ?? 0,
  ...
  DanaTalangModel copyWith({
    ...
    return DanaTalangModel(
      ...
      totalDikonversiModal:
          totalDikonversiModal ?? this.totalDikonversiModal,
