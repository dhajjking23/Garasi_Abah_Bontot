...
def diagnose() -> dict:
    ...
    if not result["file_exists"]:
        result["problem"] = (
            f"File database tidak ditemukan di '{DB_PATH}'. Untuk deployment BARU "
            "(VPS/Termux), ini normal sebelum migrasi pertama: jalankan "
            "migrate_to_master_db.py dengan file .db yang sudah ada isinya "
            "(dari HP admin, di-scp/upload ke server ini). Untuk deployment yang "
            "sudah pernah jalan, pastikan db_path di config.json belum berubah "
            "sejak migrasi terakhir dilakukan."
        )
        return result
